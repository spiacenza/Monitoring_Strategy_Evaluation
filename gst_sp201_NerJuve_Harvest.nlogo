...
to setup
 ...
 ifelse DetectA = 3.841832 [set q-std-dev 2.417518] [ifelse DetectA = -1.643709E-9 [set q-std-dev 0.41646] 
  ...
  set q 1 / (1 + (exp (- q-norm)))  set scale-factor 5.4 ; per life table
  set rh rhA ;0.15
  ...
  ifelse BreedProbVariability [set BreedProb random-gamma BPGammaShape 19.06008193] [set BreedProb BreedProbA];[set BreedProb random-normal BreedProbA 0.114 while [(BreedProb < 0) or (BreedProb >= 1.01) ] [ set BreedProb random-normal BreedProbA 0.114 ]] [set BreedProb BreedProbA]
...
ifelse Harvest_on
    [
if ticks < 200 [] 
if (ticks >= 200) and (ticks < 251) [ set Catch (round ((count NeriticJuveniles) / F)) (ask n-of Catch NeriticJuveniles [die] ) 
...
to Monitor  
  ...
  set SampleSize round(q * countNesters2)
  set Monitored-Nesters (n-of SampleSize Nesters) ;turtles with [reprostatus = "nester"]
  ;set Monitored-Nesters (max-n-of SampleSize Nesters [ClutchFrequency]) ; Monitored-Nesters sampled based on turtles with greatest clutch frequency (better chance of being observed on nesting beach
  ...
  set Monitored-Nests round(q * Nests)
 end 
...
"" ""
PENS
"Nesters" 1.0 0 -13345367 true "" "plot count turtles with [reprostatus = \"Nester\"]"
"Monitored Nesters" 1.0 0 -2674135 true "" "plot SampleSize"
...
<experiments>
  ...
  <experiment name="GST SA_Detect_FLowHigh" repetitions="50" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <timeLimit steps="351"/>
    <exitCondition>turtles = 0</exitCondition>
